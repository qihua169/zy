...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        try:
            ...
            # 提取年份（从 lastUpdate 或 publishTime 取前4位）
            year_str = ''
            last_update = v.get('lastUpdate', '')
            if last_update and len(last_update) >= 4:
                year_str = last_update[:4]
            else:
                publish_time = v.get('publishTime', '')
                if publish_time and len(publish_time) >= 4:
                    year_str = publish_time[:4]
            
            vod = {
                'vod_id': ids[0],
                'vod_name': v.get('showname', ''),
                'type_name': v.get('showcategory', '') or v.get('showVideotype', ''),
                'vod_year': year_str,
            ...
            sdata,index,director=self.getinfo(params)
    ...
    def getinfo(self,params):
        ...
        director = ''
        try:
            for node in i['nodes'][0]['nodes']:
                if node.get('type') == 20009 and node.get('nodes'):
                    # 20009 是播放页简介组件，子节点中 type=10011 的是人员
                    persons = []
                    for sub in node['nodes']:
                        if sub.get('type') == 10011:
                            title = sub.get('data', {}).get('title')
                            if title:
                                persons.append(title)
                    # 第一个人员作为导演（优酷页面通常第一个显示的是导演）
                    if len(persons) > 0:
                        director = persons[0]
                    break
        except Exception:
